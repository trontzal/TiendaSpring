...
class DaoProductoJdbcx implements DaoProductox {
	...
	public Productox obtenerPorId(Long id) {
		return jdbc.queryForObject("SELECT * FROM productos WHERE id=?",
				new BeanPropertyRowMapper<Productox>(Productox.class), id);
	}
	...
	public Productox insertar(Productox producto) {
		...
		jdbc.update(
				"INSERT INTO productos (codigo_barras, nombre, precio, fecha_caducidad, unidades) VALUES (?, ?, ?, ?, ?)",
				producto.getCodigoBarras(), producto.getNombre(), producto.getPrecio(), producto.getFechaCaducidad(),
	...
	public Productox modificar(Productox producto) {
		jdbc.update(
				"UPDATE productos SET codigo_barras=?, nombre=?, precio=?, fecha_caducidad=?, unidades=? WHERE id=?",
				producto.getCodigoBarras(), producto.getNombre(), producto.getPrecio(), producto.getFechaCaducidad(),
				producto.getUnidades(), producto.getId());
		return producto;
	}
